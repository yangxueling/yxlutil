...
public class MonitorFilter implements Filter {
	...
	public void doFilter(ServletRequest request, ServletResponse response,
		...
		((HttpServletRequest)request).getSession().setAttribute("requestTime", startTime);
		//执行
		...
		String str = "执行时间: " + runTime + "ms  (URI: " +((HttpServletRequest) request).getRequestURI() + ")";
		//大于1秒的记录下来
		if(runTime > warnTime){
			logger.warn(str);
		}
